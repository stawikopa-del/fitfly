...
export default function DayPlanner() {
  ...
  const fetchPlansCount = async () => {
    ...
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('day_plans').select('plan_date').eq('user_id', user.id).gte('plan_date', monthStart).lte('plan_date', monthEnd);
  ...
  const fetchPlans = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.from('day_plans').select('*').eq('user_id', user.id).eq('plan_date', selectedDateStr).order('order_index', {
        ascending: true
      });
      if (error) throw error;
      setPlans(data || []);
  ...
  const openEditForm = (plan: DayPlan) => {
    ...
    setPlanCoords(plan.latitude && plan.longitude ? {
      lat: plan.latitude,
      lng: plan.longitude
    } : null);
  ...
  const handleSavePlan = async () => {
    ...
    try {
      if (editingPlan) {
        ...
        const {
          error
        } = await supabase.from('day_plans').update({
          name: planName.trim(),
          time: planTime || null,
        ...
        const {
          error
        } = await supabase.from('day_plans').insert(plansToInsert);
        if (error) throw error;
        if (planRecurrence) {
  ...
  const deleteRecurringSeries = async (plan: DayPlan) => {
    ...
    try {
      ...
      const {
        error,
        count
      } = await supabase.from('day_plans').delete({
        count: 'exact'
      }).eq('user_id', user.id).eq('name', plan.name).eq('recurrence', plan.recurrence).eq('category', plan.category);
      if (error) throw error;
      fetchPlans();
      fetchPlansCount();
  ...
  const handleDragEnd = async () => {
    ...
    const updatedPlans = newPlans.map((p, idx) => ({
      ...p,
      order_index: idx
    }));
    ...
    // Save to database
    try {
      const updates = updatedPlans.map(p => supabase.from('day_plans').update({
  ...
  const renderPlanCard = (plan: DayPlan, isDraggable = false) => {
    ...
    return <div key={plan.id} data-plan-id={plan.id} draggable={isDraggable} onDragStart={() => handleDragStart(plan.id)} onDragOver={e => handleDragOver(e, plan.id)} onDragEnd={handleDragEnd} onTouchStart={() => isDraggable && handleTouchStart(plan.id)} onTouchMove={isDraggable ? handleTouchMove : undefined} onTouchEnd={isDraggable ? handleTouchEnd : undefined} className={cn('bg-card rounded-2xl p-4 border border-border/50 transition-all', plan.is_completed && 'opacity-60 bg-muted/30', draggedId === plan.id && 'opacity-50 scale-95', isDraggable && 'cursor-grab active:cursor-grabbing')}>
        <div className="flex items-start gap-3">
          {isDraggable && <div className="p-1 text-muted-foreground/50 touch-none">
              <GripVertical className="w-4 h-4" />
            </div>}
          
          <button onClick={() => toggleComplete(plan)} className={cn('w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 mt-0.5 transition-all', plan.is_completed ? 'bg-secondary border-secondary text-secondary-foreground' : 'border-muted-foreground/40 hover:border-secondary')}>
            {plan.is_completed && <Check className="w-4 h-4" />}
          </button>

          <div className="flex-1 min-w-0 cursor-pointer" onClick={() => openEditForm(plan)}>
            <div className="flex items-center gap-2 mb-1">
              <span className="text-lg">{category.icon}</span>
              <h4 className={cn('font-bold text-foreground truncate', plan.is_completed && 'line-through text-muted-foreground')}>
                {plan.name}
              </h4>
              {plan.priority === 'urgent' && <Star className="w-4 h-4 text-destructive fill-destructive shrink-0" />}
            </div>

            <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
              {plan.time && <span className="flex items-center gap-1">
                  <Clock className="w-3 h-3" />
                  {plan.time}
                </span>}
              {plan.location && <button onClick={e => {
              e.stopPropagation();
              if (plan.latitude && plan.longitude) {
                openNavigationToLocation(plan.latitude, plan.longitude);
              }
            }} className={cn("flex items-center gap-1", plan.latitude && plan.longitude && "text-primary hover:underline")}>
                  <MapPin className="w-3 h-3" />
                  {plan.location}
                  {plan.latitude && plan.longitude && <Navigation className="w-3 h-3 ml-0.5" />}
                </button>}
              <span className={cn('px-2 py-0.5 rounded-full text-[10px] font-medium', priority.bg, priority.color)}>
                {priority.label}
              </span>
              {plan.recurrence && <span className="flex items-center gap-1 text-secondary">
                  <Repeat className="w-3 h-3" />
                  {plan.recurrence === 'daily' ? 'Codziennie' : plan.recurrence === 'weekly' ? 'Co tydzień' : 'Co miesiąc'}
                </span>}
            </div>

            {plan.notes && <p className="text-xs text-muted-foreground mt-2 line-clamp-2">{plan.notes}</p>}
          </div>

          <div className="flex flex-col gap-1">
            <button onClick={() => deletePlan(plan.id)} className="p-2 text-muted-foreground hover:text-destructive transition-colors" title="Usuń ten plan">
              <Trash2 className="w-4 h-4" />
            </button>
            {plan.recurrence && <button onClick={() => deleteRecurringSeries(plan)} className="p-2 text-muted-foreground hover:text-destructive transition-colors" title="Usuń całą serię">
                <X className="w-4 h-4" />
              </button>}
          </div>
        </div>
  ...
  const formContent = <div className="space-y-4 p-4">
      {/* Nazwa planu - zawsze widoczna */}
      <div>
        <label className="text-sm font-medium text-foreground mb-1.5 block">Nazwa planu *</label>
        <Input value={planName} onChange={e => setPlanName(e.target.value)} placeholder="Co chcesz zrobić?" className="bg-background" />
      </div>

      {/* Przycisk "Więcej szczegółów" - tylko dla trybu luźne i nowych planów */}
      {mode === 'loose' && !editingPlan && <button type="button" onClick={() => setShowMoreDetails(!showMoreDetails)} className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl border border-border bg-muted/30 hover:bg-muted/50 transition-colors text-sm font-medium text-muted-foreground">
          {showMoreDetails ? <>
      ...
      <div className={cn("grid transition-all duration-300 ease-out", mode === 'template' || editingPlan || showMoreDetails ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0")}>
        <div className="overflow-hidden">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <Input type="time" value={planTime} onChange={e => setPlanTime(e.target.value)} className="bg-background" />
            </div>
            <div>
              <label className="text-sm font-medium text-foreground mb-1.5 flex items-center gap-1.5">
              ...
              <Input type="date" value={planDate} onChange={e => setPlanDate(e.target.value)} className="bg-background" />
            </div>
          </div>
          ...
          <div>
            ...
            <Input value={planLocation} onChange={e => setPlanLocation(e.target.value)} placeholder="np. Dom, Biuro" className="bg-background" />
          </div>
          ...
          <div className="space-y-2">
            ...
            {planCoords && <>
                ...
                <Button type="button" variant="outline" onClick={() => openNavigationToLocation()} className="w-full justify-start gap-2 h-11 border-secondary text-secondary hover:bg-secondary/10">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {CATEGORIES.map(cat => <button key={cat.id} type="button" onClick={() => setPlanCategory(cat.id)} className={cn('p-2 rounded-xl border text-center transition-all text-sm', planCategory === cat.id ? 'border-primary bg-primary/10 text-primary' : 'border-border bg-background hover:border-primary/50')}>
                  <span className="text-lg block mb-0.5">{cat.icon}</span>
                  <span className="text-xs">{cat.label}</span>
                </button>)}
            </div>
            {planCategory === 'custom' && <Input value={customCategory} onChange={e => setCustomCategory(e.target.value)} placeholder="Wpisz własną kategorię..." className="mt-2 bg-background" />}
          </div>
          ...
          <div>
            ...
            <div className="flex gap-2">
              {PRIORITIES.map(pri => <button key={pri.id} type="button" onClick={() => setPlanPriority(pri.id)} className={cn('flex-1 py-2 px-3 rounded-xl border text-sm font-medium transition-all', planPriority === pri.id ? `border-2 ${pri.bg} ${pri.color}` : 'border-border bg-background hover:border-primary/50')}>
                  {pri.label}
                </button>)}
            </div>
          </div>
          ...
          {!editingPlan && <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {RECURRENCE_OPTIONS.map(opt => <button key={opt.id || 'none'} type="button" onClick={() => setPlanRecurrence(opt.id)} className={cn('p-2 rounded-xl border text-center transition-all text-sm', planRecurrence === opt.id ? 'border-primary bg-primary/10 text-primary' : 'border-border bg-background hover:border-primary/50')}>
                    <span className="text-lg block mb-0.5">{opt.icon}</span>
                    <span className="text-xs">{opt.label}</span>
                  </button>)}
              </div>
              {planRecurrence && <p className="text-xs text-muted-foreground mt-2">
          ...
          <div>
            ...
            <Textarea value={planNotes} onChange={e => setPlanNotes(e.target.value)} placeholder="Dodatkowe informacje..." className="bg-background resize-none" rows={2} />
          </div>
          </div>
        </div>
      </div>
  ...
  return <div className="min-h-screen bg-background pb-24">
      ...
      <header className="sticky top-0 z-50 bg-background/95 backdrop-blur-sm border-b border-border/50 px-4 py-3">
        ...
        <div className="flex items-center justify-between mt-3 bg-muted/50 rounded-xl p-1">
          <button onClick={() => setSelectedDate(subDays(selectedDate, 1))} className="p-2 hover:bg-card rounded-lg transition-colors">
          ...
          <Popover open={calendarOpen} onOpenChange={setCalendarOpen}>
            <PopoverTrigger asChild>
              <button className="flex items-center gap-2 px-3 py-1.5 hover:bg-card rounded-lg transition-colors">
                ...
                <span className="font-medium text-sm">
                  {isToday(selectedDate) ? 'Dzisiaj' : isTomorrow(selectedDate) ? 'Jutro' : isYesterday(selectedDate) ? 'Wczoraj' : format(selectedDate, 'd MMMM', {
                  locale: pl
                })}
                </span>
            ...
            <PopoverContent className="w-auto p-0" align="center">
            ...
            }} onMonthChange={setCalendarMonth} initialFocus className="p-3 pointer-events-auto" locale={pl} modifiers={{
              ...
              DayContent: ({
                ...
                return <div className="relative w-full h-full flex items-center justify-center">
                        {date.getDate()}
                        {count > 0 && <span className="absolute -bottom-0.5 left-1/2 -translate-x-1/2 min-w-[14px] h-[14px] bg-primary text-primary-foreground text-[9px] font-bold rounded-full flex items-center justify-center">
                            {count}
                          </span>}
                      </div>;
              }
          ...
          <button onClick={() => setSelectedDate(addDays(selectedDate, 1))} className="p-2 hover:bg-card rounded-lg transition-colors">
      ...
      <div className="px-4 py-4 space-y-4">
        ...
        <div className="bg-muted/50 rounded-2xl p-1 flex">
          <button onClick={() => setMode('loose')} className={cn('flex-1 py-2.5 px-4 rounded-xl font-medium text-sm transition-all', mode === 'loose' ? 'bg-card shadow-sm text-foreground' : 'text-muted-foreground hover:text-foreground')}>
            📝 Luźne
          </button>
          <button onClick={() => setMode('template')} className={cn('flex-1 py-2.5 px-4 rounded-xl font-medium text-sm transition-all', mode === 'template' ? 'bg-card shadow-sm text-foreground' : 'text-muted-foreground hover:text-foreground')}>
            📋 Szablon
          </button>
        </div>

        {/* Progress */}
        {plans.length > 0 && <div className="bg-card rounded-2xl p-4 border border-border/50">
            ...
            {progressPercent === 100 && <p className="text-xs text-secondary mt-2 flex items-center gap-1">
        ...
        {mode === 'loose' && plans.length > 1 && <p className="text-xs text-muted-foreground text-center flex items-center justify-center gap-1">
            <GripVertical className="w-3 h-3" />
            Przeciągnij, aby zmienić kolejność
          </p>}

        {/* Content */}
        {mode === 'loose' ? <div className="space-y-3">
            {loading ? <div className="flex justify-center py-8">
                <div className="animate-spin w-6 h-6 border-2 border-primary border-t-transparent rounded-full" />
              </div> : plans.length === 0 ? <div className="text-center py-12">
              ...
              </div> : plans.map(plan => renderPlanCard(plan, true))}
          </div> : <div className="space-y-4">
            {TIME_SECTIONS.map(section => {
          const sectionPlans = plans.filter(p => p.time_of_day === section.id);
          const SectionIcon = section.icon;
          return <div key={section.id} className={cn('rounded-2xl border border-border/50 overflow-hidden bg-gradient-to-br', section.color)}>
                  <div className="p-4 flex items-center justify-between">
                    ...
                    <Sheet>
                      <SheetTrigger asChild>
                        <Button size="sm" variant="ghost" className="rounded-xl" onClick={() => setEditingTimeOfDay(section.id)}>
                  ...
                  {sectionPlans.length > 0 && <div className="px-4 pb-4 space-y-2">
                      {sectionPlans.map(plan => renderPlanCard(plan, false))}
                    </div>}
                  
                  {sectionPlans.length === 0 && <div className="px-4 pb-4">
      ...
      {/* FAB for Loose mode */}
      {mode === 'loose' && <Sheet open={isAddingPlan} onOpenChange={open => {
      ...
      {/* Map Picker Modal */}
      {showMapPicker && <Suspense fallback={<div className="fixed inset-0 z-50 bg-background flex items-center justify-center">
            <div className="animate-spin w-8 h-8 border-4 border-primary border-t-transparent rounded-full" />
          </div>}>
          <LocationPickerMap onSelectLocation={handleLocationSelect} onClose={() => setShowMapPicker(false)} />
        </Suspense>}
    </div>;
}