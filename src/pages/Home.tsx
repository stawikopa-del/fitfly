...
const getTimeGreeting = () => {
  ...
  if (hour >= 5 && hour < 12) return { text: 'Dzień dobry', emoji: '🌅' };
  if (hour >= 12 && hour < 18) return { text: 'Cześć', emoji: '☀️' };
  if (hour >= 18 && hour < 22) return { text: 'Dobry wieczór', emoji: '🌙' };
  return { text: 'Nocny marek?', emoji: '🦉' };
};
...
export default function Home() {
  ...
  const { challenges, habits, todayLogs, loading: habitsLoading } = useHabitsAndChallenges();
    ...
    const fetchProfile = async () => {
      try {
        const { data, error } = await supabase
          .from('profiles')
          .select('display_name, daily_calories')
          .eq('user_id', user.id)
          .maybeSingle();
    ...
    <div className="px-4 py-6 space-y-6">
      ...
      {!gamificationLoading && gamification && (
        <section className="relative z-10">
          <div 
            className="bg-gradient-to-r from-accent/20 via-primary/10 to-secondary/20 rounded-2xl p-4 border border-accent/30 cursor-pointer hover:shadow-md transition-all"
      ...
      <section className="grid grid-cols-2 gap-4 relative z-10">
        ...
        <div className="animate-float" style={{ animationDelay: '0.6s' }}>
          <StatCard
            ...
            subValue={`/ ${caloriesGoal.toLocaleString()} kcal`}
            color="purple"
            onClick={() => navigate('/odzywianie')}
          />
        </div>
        <div className="animate-float" style={{ animationDelay: '0.9s' }}>
          <StatCard
            ...
            label="Nawyki"
            value={`${completedHabitsToday}/${totalActiveHabits}`}
            subValue={totalActiveHabits > 0 ? 'wykonane dziś' : 'brak nawyków'}
            color="blue"
      ...
      <section className="space-y-4 relative z-10">
        ...
        <div className="space-y-3">
          ...
          <div className="animate-float" style={{ animationDelay: '1.4s' }}>
            <QuickAction
              icon={<img src={activeChallenge ? fitekPuchar : fitekPiatka} alt="FITEK" className="w-8 h-8 object-contain" />}
              title={activeChallenge ? activeChallenge.title : 'Wyzwanie dnia'}
              description={
                activeChallenge 
                  ? `${activeChallenge.current}/${activeChallenge.target} ${activeChallenge.unit} - ${Math.round((activeChallenge.current / activeChallenge.target) * 100)}% ukończone`
