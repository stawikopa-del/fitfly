...
export default function Auth() {
  ...
  const { isSupported: isBiometricSupported, hasRegisteredBiometric, authenticateWithBiometric, isAuthenticating } = useWebAuthn();
      ...
      <div className="text-center mb-8">
        <h1 className="text-5xl font-extrabold font-display mb-3">
          {mode === 'forgot' ? 'Reset hasła' : (
            <>
              <span style={{ color: '#85c530' }}>FITFLY</span>
            ...
            {mode === 'login' && (
              <>
                ...
                <div className="space-y-3">
                  ...
                  {isBiometricSupported && hasRegisteredBiometric() && (
                    <Button 
                      type="button" 
                      variant="outline" 
                      onClick={handleBiometricLogin} 
                      disabled={isLoading || isAuthenticating}
                      className="w-full h-12 rounded-2xl font-bold text-base border-2 gap-3 border-primary/30 hover:border-primary/50 hover:bg-primary/5"
                    >
                      {isAuthenticating ? (
                        <span className="flex items-center gap-2">
                          <span className="animate-spin w-5 h-5 border-2 border-current border-t-transparent rounded-full" />
